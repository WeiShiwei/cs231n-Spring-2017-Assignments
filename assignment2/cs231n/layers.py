...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        # running_mean = momentum*running_mean + (1.0-momentum)*running_mean
        # running_var = momentum*running_var + (1.0-momentum)*running_var
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    # http://blog.csdn.net/xiaojiajia007/article/details/54924959
...
def max_pool_forward_naive(x, pool_param):
    ...
    for n in range(N):
        for c in range(C):
            for hh in range(Hh):
                for ww in range(Ww):
                    sub_x = x[n,c,hh*S:(hh+1)*S,ww*S:(ww+1)*S]
                    out[n,c,hh,ww] = np.max(sub_x)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
